...
(function() {
  ...
  globalStyle.textContent = `
    ...
    @keyframes slideUpHint{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
  `;
  ...
  document.addEventListener('click', function(e) {
    ...
    if (btn) {
      ...
      const name = btn.querySelector('.tool-name')?.textContent?.trim();
      if (name) {
  ...
  const hintSeen = localStorage.getItem('francis_hint_seen');
  if (!hintSeen) {
    localStorage.setItem('francis_hint_seen', '1');
    setTimeout(() => {
      ...
      hint.style.cssText = `
        ...
        z-index:8888;animation:slideUpHint .3s ease;
        cursor:pointer;
      `;
